...
class BookSpider(scrapy.Spider):
    ...
    def book_list(self, response):  # 书籍列表页
        ...
        for li in li_list:
            item["p_name"] = li.xpath(".//div[@class='p-name']/a/em/text()").extract_first().strip()  # 有空白字符，使用strip(),注意对象必须是字符串

            item["p-img"] = li.xpath(".//div[@class='p-img']/a/img/@src").extract_first()
            # if item["p-img"] is None:
            #     item["p-img"] = "https:" + li.xpath(".//div[@class='p-img']/a/img/@title").extract_first()
            if item["p-img"] is not None:  # 判断链接是否为None
                item["p-img"] = "https:" + item["p-img"]

            item["p-author"] = [i.strip() for i in li.xpath(".//div[@class='p-bookdetails']/span[1]//text()").extract() if len(i)>0]
            # extract() 得到列表--> len()>0 剔除空字符串  --> i.strip() 剔除左右的空白字符
